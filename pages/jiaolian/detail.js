...
const {
data
} = await detail("jiaolian",id)
this.setData({
payAuth:utils.isAuthFront('jiaolian','支付')
})

const detailList = data
this.setData({
detailList,
picture: detailList.zhaopian.split(','),
})
...
const resUpdate = await update('jiaolian', predetailList)
if (resUpdate.code == 0) {
this.setData({
predetailList,
...
wx.setStorageSync('paytable','jiaolian');
...
async reply() {
const detailList = this.data.detailList
const res = await update("jiaolian", detailList)
if (res.code == 0) {
setTimeout(function () {
wx.showToast({
title: '回复成功',
