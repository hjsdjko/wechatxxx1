...
this.setData({
addAuth: utils.isAuth("jiaolian", "新增") || utils.isAuthFront("jiaolian", "新增"),
delAuth: utils.isAuth("jiaolian", "删除") || utils.isAuthFront("jiaolian", "删除"),
editAuth: utils.isAuth("jiaolian", "修改") || utils.isAuthFront("jiaolian", "修改"),
})
if(options?.userid) {
this.setData({
...
page: this.data.currPage,
    limit: this.data.pageSize
            ...
            searchForm['jiaolianzhanghao'] = '%' + this.data.jiaolianzhanghao + '%'
        }
    

let res = {};
if(this.data.userid) {
res = await page(`jiaolian`, searchForm);
} else {
res = await list(`jiaolian`, searchForm);
}
...
wx.navigateTo({
url: `/pages/jiaolian/update-and-add`
})
},
searchListHandler(e) {
this.setData({
...
complete: async (res) => {
    ...
    if (res.confirm) {
        ...
        const res = await deleteData("jiaolian",[id])
        console.log(res);
        if (res.code == 0) {
            this.getData()
        }
