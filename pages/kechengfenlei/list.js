...
this.setData({
addAuth: utils.isAuth("kechengfenlei", "新增") || utils.isAuthFront("kechengfenlei", "新增"),
delAuth: utils.isAuth("kechengfenlei", "删除") || utils.isAuthFront("kechengfenlei", "删除"),
editAuth: utils.isAuth("kechengfenlei", "修改") || utils.isAuthFront("kechengfenlei", "修改"),
})
if(options?.userid) {
this.setData({
...
page: this.data.currPage,
    limit: this.data.pageSize
            ...
            if(this.data.kechengfenlei){
            searchForm['kechengfenlei'] = '%' + this.data.kechengfenlei + '%'
        }
    

let res = {};
if(this.data.userid) {
res = await page(`kechengfenlei`, searchForm);
} else {
res = await list(`kechengfenlei`, searchForm);
}
...
wx.navigateTo({
url: `/pages/kechengfenlei/update-and-add`
})
},
searchListHandler(e) {
this.setData({
...
complete: async (res) => {
    ...
    if (res.confirm) {
        ...
        const res = await deleteData("kechengfenlei",[id])
        console.log(res);
        if (res.code == 0) {
            this.getData()
        }
...
wx.navigateTo({
url: `/pages/kechengfenlei/detail`
})
