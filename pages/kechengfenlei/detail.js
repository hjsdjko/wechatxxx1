...
const {
data
} = await detail("kechengfenlei",id)
this.setData({
payAuth:utils.isAuthFront('kechengfenlei','支付')
})
...
const resUpdate = await update('kechengfenlei', predetailList)
if (resUpdate.code == 0) {
this.setData({
predetailList,
...
wx.setStorageSync('paytable','kechengfenlei');
...
async reply() {
const detailList = this.data.detailList
const res = await update("kechengfenlei", detailList)
if (res.code == 0) {
setTimeout(function () {
wx.showToast({
title: '回复成功',
