...
this.setData({
addAuth: utils.isAuth("discusskechengxinxi", "新增") || utils.isAuthFront("discusskechengxinxi", "新增"),
delAuth: utils.isAuth("discusskechengxinxi", "删除") || utils.isAuthFront("discusskechengxinxi", "删除"),
editAuth: utils.isAuth("discusskechengxinxi", "修改") || utils.isAuthFront("discusskechengxinxi", "修改"),
})
if(options?.userid) {
this.setData({
...
page: this.data.currPage,
    limit: this.data.pageSize
            ...
            searchForm['nickname'] = '%' + this.data.nickname + '%'
        }
                if(this.data.content){
            searchForm['content'] = '%' + this.data.content + '%'
        }
    

let res = {};
if(this.data.userid) {
res = await page(`discusskechengxinxi`, searchForm);
} else {
res = await list(`discusskechengxinxi`, searchForm);
}
...
wx.navigateTo({
url: `/pages/discusskechengxinxi/update-and-add`
})
},
searchListHandler(e) {
this.setData({
...
complete: async (res) => {
    ...
    if (res.confirm) {
        ...
        const res = await deleteData("discusskechengxinxi",[id])
        console.log(res);
        if (res.code == 0) {
            this.getData()
        }
...
wx.navigateTo({
url: `/pages/discusskechengxinxi/detail`
})
...
const {
    data
}= await page("discusskechengxinxi",obj)
...
const {
    data
}= await list("discusskechengxinxi",obj)
totalPage=data?.totalPage
