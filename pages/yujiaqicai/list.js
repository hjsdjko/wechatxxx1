...
this.setData({
addAuth: utils.isAuth("yujiaqicai", "新增") || utils.isAuthFront("yujiaqicai", "新增"),
delAuth: utils.isAuth("yujiaqicai", "删除") || utils.isAuthFront("yujiaqicai", "删除"),
editAuth: utils.isAuth("yujiaqicai", "修改") || utils.isAuthFront("yujiaqicai", "修改"),
})
if(options?.userid) {
this.setData({
...
page: this.data.currPage,
    limit: this.data.pageSize
            ...
            if(this.data.qicaimingcheng){
            searchForm['qicaimingcheng'] = '%' + this.data.qicaimingcheng + '%'
        }
    

let res = {};
if(this.data.userid) {
res = await page(`yujiaqicai`, searchForm);
} else {
res = await list(`yujiaqicai`, searchForm);
}
...
goodsListData=res.data.list.map(item=>{
                    item.qicaitupian = item.qicaitupian.split(",")[0];
                return item
})
...
wx.navigateTo({
url: `/pages/yujiaqicai/update-and-add`
})
},
searchListHandler(e) {
this.setData({
...
complete: async (res) => {
    ...
    if (res.confirm) {
        ...
        const res = await deleteData("yujiaqicai",[id])
        console.log(res);
        if (res.code == 0) {
            this.getData()
        }
...
const {
    data
}= await page("yujiaqicai",obj)
totalPage=data?.totalPage
goodsListData = data?.list.map(item => {
item.qicaitupian? item.qicaitupian = item.qicaitupian.split(",")[0].replace('upload','flie'):""
    return item;
});
}
else{
