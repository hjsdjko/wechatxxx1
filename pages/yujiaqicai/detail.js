...
tablename: 'yujiaqicai',
userid:userInfo.id,
goodid: this.data.detailList.id,
    goodname:  this.data.detailList.qicaimingcheng,
...
const {
data
} = await detail("yujiaqicai",id)
this.setData({
payAuth:utils.isAuthFront('yujiaqicai','支付')
})

const detailList = data
this.setData({
detailList,
picture: detailList.qicaitupian.split(','),
})
...
const resUpdate = await update('yujiaqicai', predetailList)
if (resUpdate.code == 0) {
this.setData({
predetailList,
...
data["qicaitupian"] = data.qicaitupian
data['buynumber']=1
wx.setStorageSync('payObject',data);
wx.setStorageSync('paytable','yujiaqicai');
...
async reply() {
const detailList = this.data.detailList
const res = await update("yujiaqicai", detailList)
if (res.code == 0) {
setTimeout(function () {
wx.showToast({
title: '回复成功',
